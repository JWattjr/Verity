...
contract VerityFPMMTest is Test {
  ...
  function test_escrowAggregatesAndDeploys() public {
    ...
    assertEq(
      fpmm.lpShares(marketId, creator),
      0,
      "Creator deposited exactly 10, all of it is locked"
    );
  ...
  function test_creatorSharesAreLocked() public {
    ...
    assertEq(
      fpmm.lpShares(marketId, creator),
      0,
      "Creator normal shares should be 0"
    );
  ...
  function test_addLiquidityRecordsTimestamp() public {
    ...
    assertEq(
      fpmm.lpDepositTime(marketId, lp1),
      depositTime,
      "Deposit time should be recorded"
    );
  }
  ...
  function test_buyDeducts2PercentFee() public {
    ...
    assertEq(
      ...
      expectedTreasuryFee,
      "Treasury should receive 40% of fee"
    );
  }
  ...
  function test_pricesSumToOne() public {
    ...
    assertEq(
      yesPrice + noPrice,
      1e18,
      "Prices should still sum to 1e18 after trade"
    );
  }
  ...
  function test_sellReturnsUsdc() public {
    ...
    assertEq(
      usdc.balanceOf(trader) - usdcBefore,
      usdcOut,
  ...
  function test_sellDeductsFee() public {
    ...
    assertTrue(
      treasuryAfter > treasuryBefore,
      "Treasury should receive fee from sell"
    );
  }
  ...
  function test_largeTradeSlippage() public {
    ...
    assertTrue(
      yesPriceAfter > 75e16,
      "YES price should be >75% after large buy"
    );
  }
  ...
  function test_claimCreatorLiquidityAfterResolution() public {
    ...
    assertEq(
      fpmm.lpShares(marketId, creator),
      0,
      "Creator shares should be zero after claim"
    );
    ...
    uint256 noId = vault.noTokenId(marketId);
    assertEq(
      vault.balanceOf(creator, yesId),
      0,
      "Creator should hold 0 YES tokens"
    );
    assertEq(
      vault.balanceOf(creator, noId),
      0,
      "Creator should hold 0 NO tokens"
    );
  }
  ...
  function test_multipleSequentialTrades() public {
    ...
    assertEq(
      yesPrice + noPrice,
      1e18,
      "Prices should sum to 1e18 after multiple trades"
    );
  }
  ...
  function test_canRemoveLiquidityView() public {
    ...
    assertFalse(
      fpmm.canRemoveLiquidity(marketId, lp1),
      "Should not be able to remove within 24h"
    );

    // Creator: should be false (locked until resolution)
    assertFalse(
      fpmm.canRemoveLiquidity(marketId, creator),
      "Creator should not be able to remove"
    );
    ...
    assertTrue(
      fpmm.canRemoveLiquidity(marketId, lp1),
      "Should be able to remove after 24h"
    );

    // Creator still locked
    assertFalse(
      fpmm.canRemoveLiquidity(marketId, creator),
      "Creator should still be locked"
    );
    ...
    assertTrue(
      fpmm.canRemoveLiquidity(marketId, creator),
      "Creator should be able to remove after resolution"
    );
  }
