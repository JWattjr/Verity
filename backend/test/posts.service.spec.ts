...
import { PostsService, MARKET_OUTCOME_WARNING } from "../src/modules/posts/posts.service";
...
describe("PostsService", () => {
  ...
  describe("getMarketWarning", () => {
    it("should return warning if question has vague words", () => {
      expect(service.getMarketWarning("Will this become popular?")).toBe(MARKET_OUTCOME_WARNING);
    });
  ...
  describe("createNormalPost", () => {
    ...
    it("should throw NotFoundException if author does not exist", async () => {
      ...
      await expect(service.createNormalPost(mockUser.id, "Hello")).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("createMarketPost", () => {
    it("should throw UnprocessableEntityException if missing creationFeeTxHash", async () => {
      userModel.exists.mockResolvedValue(true);
      await expect(service.createMarketPost(mockUser.id, {
