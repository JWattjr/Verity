...
describe("BlockchainService", () => {
  ...
  beforeEach(async () => {
    const mockConfigService = {
      get: jest.fn().mockImplementation((key: string) => {
        ...
        if (key === "TEST_PRIVATE_KEY" || key === "KEEPER_PRIVATE_KEY") {
  ...
  describe("formatMarketId", () => {
    it("should format Mongo ID string into bytes32 padded hex string", () => {
      ...
      expect(formatted).toBe("0x60d0fe4f5311236168a109ca0000000000000000000000000000000000000000");
    });
  });
  ...
  describe("getTransactionReceipt", () => {
    it("should return the transaction receipt", async () => {
      ...
      expect((service as any).publicClient.waitForTransactionReceipt).toHaveBeenCalledWith({
        hash: "0xTxHash",
      });
    });
  });
  ...
  describe("readOnChainMarketState", () => {
    it("should query publicClient readContract and return results", async () => {
      ...
      const result = await service.readOnChainMarketState("60d0fe4f5311236168a109ca");
