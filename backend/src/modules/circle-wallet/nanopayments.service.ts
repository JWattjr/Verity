...
export class NanopaymentsService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (!rawPrivateKey) {
      this.logger.warn(
        "Nanopayments Service: ADMIN_PRIVATE_KEY or KEEPER_PRIVATE_KEY is not configured.",
      )
      return
    }
    ...
    try {
      ...
      this.logger.log(
        `Circle GatewayClient initialized successfully on chain: ${chain}`,
      )
    } catch (error: any) {
  ...
  async payoutUSDC(
    ...
    try {
      ...
      this.logger.log(
        `Circle Gateway balance: ${availableGatewayUsdc} USDC (Available), Wallet balance: ${Number(balances.wallet.balance) / 1e6} USDC`,
      )
      ...
      if (availableGatewayUsdc < amountUsdc) {
        ...
        this.logger.log(
          `Depositing ${depositAmount} USDC from Treasury wallet to Circle Gateway...`,
        )
        const depResult = await this.client.deposit(depositAmount.toFixed(6))
        this.logger.log(
          `USDC Deposit transaction submitted: ${depResult.depositTxHash}. Waiting 3 seconds...`,
        )
      ...
      this.logger.log(
        `Executing off-chain Circle Gateway withdrawal of ${amountUsdc} USDC to ${recipientAddress}`,
      )
      const res = await this.client.withdraw(amountUsdc.toFixed(6), {
        recipient: recipientAddress as `0x${string}`,
      })
      this.logger.log(
        `Circle Gateway payout complete. Mint TX hash: ${res.mintTxHash}`,
      )
      ...
      throw new BadRequestException(
        `Circle Gateway payout failed: ${error.message || error}`,
      )
    }
  }
}
