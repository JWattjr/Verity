...
export class CircleWalletService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (!apiKey || !entitySecret) {
      this.logger.warn("Circle Wallet Service: CIRCLE_API_KEY or CIRCLE_ENTITY_SECRET is not configured.");
      return;
    }
    ...
    try {
      ...
      this.logger.log("Circle WaaS Developer Controlled Wallets Client initialized successfully.");
    } catch (error) {
  ...
  async createWalletForUser(userId: string): Promise<string> {
    ...
    if (!walletSetId) {
      throw new BadRequestException("CIRCLE_WALLET_SET_ID is not configured in .env");
    }

    try {
      this.logger.log(`Requesting Circle WaaS SCA Wallet on ARC-TESTNET for user: ${userId}`);
      ...
      this.logger.log(`SCA Wallet created successfully: ${wallet.address} (ID: ${wallet.id})`);
      ...
      this.logger.error(`Failed to create Circle wallet for user ${userId}:`, error.response?.data || error.message);
      throw new BadRequestException(`Circle wallet creation failed: ${error.response?.data?.message || error.message}`);
    }
  }
  ...
  async executeContractCall(
    ...
    try {
      this.logger.log(`Executing contract call to ${contractAddress} [${abiFunctionSignature}] with parameters: ${JSON.stringify(abiParameters)}`);
      ...
      const details = error.response?.data ? JSON.stringify(error.response.data, null, 2) : (error.message || error);
      this.logger.error(`Contract execution transaction failed: ${details}`);
      throw new BadRequestException(`Contract execution failed: ${error.message || error}`);
    }
  }

  async executeBatch(
    userId: string,
    calls: Array<{ contractAddress: string; abiFunctionSignature: string; abiParameters: any[] }>,
    estimatedCostUsdc?: number,
  ): Promise<string> {
    const user = await this.userModel.findById(userId);
    if (!user || !user.circleWalletId || !user.walletAddress) {
      throw new BadRequestException("User does not have an active Circle Developer-Controlled Wallet.");
    }
    ...
    this.logger.log(`Starting execution of batch with ${calls.length} calls for user ${userId}`);
    ...
    const usdcAddress = this.configService.get<string>("USDC_ADDRESS") || "0x3600000000000000000000000000000000000000";
    ...
    try {
      const nativeBalance = await publicClient.getBalance({ address: user.walletAddress as `0x${string}` });
      if (nativeBalance === 0n) {
        throw new BadRequestException("Insufficient native ARC tokens for gas. Please request ARC from the faucet.");
      }
    } catch (err: any) {
    ...
    const encodedCalls = calls.map((call) => {
      ...
      const signature = rawSig.startsWith("function ") ? rawSig : `function ${rawSig}`;
    ...
    this.logger.log(`Encoded calls for batch: ${JSON.stringify(encodedCalls, null, 2)}`);
  ...
  private async pollTransactionReceipt(transactionId: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        this.logger.debug(`Polling transaction ${transactionId} state: ${state} (Attempt ${attempt}/${maxAttempts})`);

        if (state === "COMPLETE" || state === "CONFIRMED") {
          const txHash = tx?.txHash || response.data?.txHash;
          if (!txHash) {
            throw new Error("Transaction state is complete but no on-chain txHash was found.");
          }
          return txHash;
        }
        ...
        this.logger.warn(`Error polling transaction ${transactionId}: ${error.message}`);
      }
    }

    throw new Error(`Transaction ${transactionId} did not reach complete state within 60 seconds.`);
  }
}
