...
async function run() {
  ...
  const factoryPath = path.join(
    contractsDir,
    "out/VerityMarketFactory.sol/VerityMarketFactory.json",
  )
  ...
  if (fs.existsSync(factoryPath)) {
    ...
    console.error(
      `✗ VerityMarketFactory.json not found at expected path: ${factoryPath}`,
    )
  }
  ...
  if (extractedCount === totalToExtract) {
    ...
    console.warn(
      `=== Extraction finished with issues: only ${extractedCount}/${totalToExtract} ABIs found ===`,
    )
    process.exit(1)
  }
}
