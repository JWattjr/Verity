  ...
  return (
    <span className="group/user-card relative inline-flex min-w-0">
      ...
      <span className="pointer-events-none absolute left-0 top-full z-40 hidden w-[min(320px,calc(100vw-2rem))] pt-2 group-hover/user-card:block group-focus-within/user-card:block">
        <span className="pointer-events-auto block rounded-[2px] bg-surface-solid p-4 shadow-sm">
          <span className="mb-3 flex items-start justify-between gap-3">
            <Link
              className="clickable-surface flex min-w-0 items-center gap-3 rounded-[2px] p-1"
              href={profileHref}
            >
...
function HoverStat({ label, value }: { label: string; value: string }) {
  return (
    <span className="rounded-[2px] bg-surface-muted p-2 shadow-subtle">
...
function ProfileAvatar({ profile }: { profile: Profile }) {
  ...
  if (avatarUrl) {
    return (
      <span
        className="h-12 w-12 shrink-0 rounded-[2px] bg-cover bg-center shadow-subtle"
